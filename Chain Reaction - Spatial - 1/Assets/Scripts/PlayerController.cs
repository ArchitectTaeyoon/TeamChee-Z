    ...
    public void ScoreCalculator()
    {
        ...
        for (int i = 0; i < length; i++)
        {
            for (int j = 0; j < breadth; j++)
            {
                ...
                {
                    int occupyingPlayer = GameBoardArray[i, j].GetComponent<GridUnit>().FindOccupyingPlayer();
                    int numberofSpheres = GameBoardArray[i, j].GetComponent<GridUnit>().GetNumberOfSpheres();
                    PlayerArray[occupyingPlayer].SetScore(PlayerArray[occupyingPlayer].GetScore()+numberofSpheres);
                }
            }
        }
    }
