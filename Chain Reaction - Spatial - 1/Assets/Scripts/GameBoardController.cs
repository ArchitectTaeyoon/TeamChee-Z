    ...
    void CreateBoard(){
        ...
        for (int i=0;i<length;i++) {
            for (int j = 0; j < breadth; j++) {
                ...
                //Instantiate floor plane inside the grid
                GameObject floor = Instantiate(floorPlanePrefab, new Vector3(i, 0.5f, j), Quaternion.identity);
                floor.name = "Floor(" + i.ToString() + "," + j.ToString() + ")";
                floor.GetComponent<MeshRenderer>().enabled = false;
                floor.transform.parent = GameBoardArray[i, j].transform;
                //Instantiate walls inside the grid
                GameObject walls = Instantiate(wallsPrefab, new Vector3(i, 0, j), Quaternion.identity);
                walls.name= "Walls(" + i.ToString() + "," + j.ToString() + ")";
                walls.transform.parent= GameBoardArray[i, j].transform;
