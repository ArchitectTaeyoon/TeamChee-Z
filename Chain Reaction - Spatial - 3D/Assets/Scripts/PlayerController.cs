...
public class PlayerController : MonoBehaviour {
    ...
    //Set Player IDS
    void SetPlayerIDsandMoney(){
        for (int i = 0; i < PlayerArray.Length; i++) {
            PlayerArray[i].PlayerID = i;
            PlayerArray[i].Money = StartingMoney;
        }
    }
    ...
    public void ScoreCalculator()
    {
        ...
        for (int i = 0; i < length; i++)
        {
            for (int j = 0; j < breadth; j++)
            {
                try
                {
                    for(int k=0;k< GameBoardArray[i, j].GetComponent<BoardGrid>().VerticalNeighbourhood.Length; k++)
                    {
                        if(GameBoardArray[i, j].GetComponent<BoardGrid>().VerticalNeighbourhood[k].GetComponent<GridUnit>().IsOccupied())
                        {
                            for (int x = 0; x < GameBoardArray[i, j].GetComponent<BoardGrid>().VerticalNeighbourhood[k].GetComponent<GridUnit>().MaximumCapacity; x++)
                            {
                                int currentPlayer = GameBoardArray[i, j].GetComponent<BoardGrid>().VerticalNeighbourhood[k].GetComponent<GridUnit>().occupyingPlayers[x];
                                if (currentPlayer != -1)
                                {
